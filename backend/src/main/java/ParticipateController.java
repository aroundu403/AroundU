...
public class ParticipateController {
     ...
     */
    public static ArrayList<String> getUsersByEvent (DataSource pool, long event_id) throws SQLException {
        ArrayList<String> userIDs = new ArrayList<>();
        try (Connection conn = pool.getConnection()) {
            String stmt = String.format(
                    "SELECT user_id FROM %s WHERE event_id = ?;",
                    TABLE_NAME);
     ...
     */
    public static ArrayList<Long> getEventsByUser (DataSource pool, String user_id) throws SQLException {
        ArrayList<Long> eventIDs = new ArrayList<>();
        try (Connection conn = pool.getConnection()) {
            String stmt = String.format(
                    "SELECT event_id FROM %s WHERE user_id = ?;",
                    TABLE_NAME);
    ...
    public static int countParticipants (DataSource pool, long event_id) throws SQLException {
        ...
        try (Connection conn = pool.getConnection()) {
            String stmt = String.format(
                    "SELECT COUNT(user_id) FROM %s WHERE event_id = ?;",
                    TABLE_NAME);
